  ...
  <div class="container">
    ...
    <div class="form-group">
      <div class="row mt-5">
        ...
        <div class="col">
          <input id="startDate" class="form-control" type="date" ref="startDate" @change="setStartDate" />
        </div>
        <div class="col-auto">
          <label for="endDate" class="col-form-label">To:</label>
          </div>
        <div class="col">
          <input id="endDate" class="form-control" type="date" ref="endDate" @change="setEndDate" />
        </div>
      </div>
      <div class="row mt-4">
        <div class="col-auto">
        ...
        <div class="col">
          <input id="searchText" type="text" class="form-control" ref="searchText" @change="setSearch" />
        </div>
        <div class="col-auto">
    ...
    <div class="row">
      <div class="col">
        <button class="btn btn-primary" @click="nextPage">Prev</button>
      </div>
...
export default {
  ...
  mounted() {
    ...
    .then(data => {
      ...
      this.$refs.endDate.value = data["max-date"].substring(0,10)
  ...
  methods: {
    getData() {
      fetch('http://localhost:8000/mongo-orders/'+this.page.toString())
    .then(res => res.json())
    .then(data => {
      this.orders = data
      return fetch('http://localhost:8000/post-orders/'+this.page.toString())
    })
    .then(res => res.json())
    .then(data => {
      for (let i=0; i<data.length; i++) {
        if (data[i]["delivered-amount"] > 0)
          this.orders[i]["delivered-amount"] = data[i]["delivered-amount"].toFixed(2)
        else
